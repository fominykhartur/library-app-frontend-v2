        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td v-if="isMy">
              <v-btn
                min-width="150"
                @click="statusChange(item.columns.id, item.columns.status)"
                >{{
                  item.columns.status == 1 ? "Прочитано" : "Непрочитано"
                }}</v-btn
              >
            </td>
          </tr>
...
export default {
  data() {
    return {
      ...
      headers: [
        { key: "id", title: "ID" },
        { key: "book_name", title: "Название книги" },
  ...
  methods: {
    getUsername: function () {
      postRequest(`${import.meta.env.VITE_HOST}/library-api/public/username`, {
    ...
    getUserBooks: function () {
      this.loading = true;
      postRequest(`${import.meta.env.VITE_HOST}/library-api/public/userBooks`, {
        id: this.$route.params.id,
      }).then((res) => {
      ...
      postRequest(
        `${import.meta.env.VITE_HOST}/library-api/public/userCategories`,
        { id: this.$route.params.id }
      ).then((res) => {
        this.categories = [
          "Все",
          ...res.data.map((item: any) => item.category_name),
        ];
    ...
    categoryChange: function () {
      ...
      console.log(
        this.userbooks.filter((item: any) => item.category_name === this.select)
      );
      if (this.select === "Все") {
        this.filteredBooks = this.userbooks;
      } else {
        this.filteredBooks = this.userbooks.filter(
          (item: any) => item.category_name === this.select
        );
      }
    },
    statusChange: function (id: number, status: number) {
      ...
      status = +!status;
      postRequest(
        `${import.meta.env.VITE_HOST}/library-api/users/changeBookStatus`,
        { id, status }
      ).then((res) => {
        console.log(res);
        this.getUserBooks();
