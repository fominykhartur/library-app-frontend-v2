  ...
  <v-row justify="center">
    <v-dialog v-model="dialog" persistent>
      <template v-slot:activator="{ props }">
        <v-btn color="primary" v-bind="props"> Добавить книгу </v-btn>
      </template>
      <v-card>
        <v-card-title>
...
export default {
  ...
  methods: {
    searchPreparedBook: function () {
      ...
      if (
        Array.from(
          this.newBooks.filter((item: any) => item.book_name == this.inputName)
        ).length > 0
      ) {
        this.inputAuthor = Array.from(
          this.newBooks.filter((item: any) => item.book_name == this.inputName)
        )[0]?.author_name;
        this.inputAuthorDisable = true;
        this.inputCategory = Array.from(
          this.newBooks.filter((item: any) => item.book_name == this.inputName)
        )[0]?.category_name;
    ...
    removeUserBooks: function (books: any[]) {
      this.newBooks = Array.from(
        books.filter((newBook: any) => {
          return this.userbooks.every((oldBook: any) => {
            return newBook.book_id !== oldBook.book_id;
          });
        })
      );
      this.newBooksNames = Array.from(
        this.newBooks.map((item: any) => item.book_name)
      );
      this.newBooksAuthors = Array.from(
        new Set(this.newBooks.map((item: any) => item.author_name))
      );
      this.newBooksCategories = Array.from(
        new Set(this.newBooks.map((item: any) => item.category_name))
      );
    },
    ...
    addBookToUser: function () {
      postRequest(`${import.meta.env.VITE_HOST}/library-api/users/addNewBook`, {
    ...
    getCategories: function () {
      getRequest(
        `${import.meta.env.VITE_HOST}/library-api/books/categories`
      ).then((res) => {
        this.allCategories = res.data.map((item: any) => item.category_name);
      });
    },
    getAuthors: function () {
      getRequest(`${import.meta.env.VITE_HOST}/library-api/books/authors`).then(
        (res) => {
